...
replacement = '''  const perceptionV6 = analyzePerceptionV6({ perceptionV5, regions: garmentEvidenceRegions, decodedImage, mode: perceptionV6Mode });
...
pattern = re.compile(
    r'  const perceptionV6 = analyzePerceptionV6\(\{ perceptionV5, regions: garmentEvidenceRegions, decodedImage, mode: perceptionV6Mode \}\);\n'
    r'.*?'
    r'  const garmentZones = perceptionV6Mode === "shadow" \? legacyGarmentZones : \{\n'
    r'.*?'
    r'    v6_publication_diagnostics: perceptionV6\.publication_decisions,\n'
    r'  \};\n',
    re.S,
)
...
if count != 1:
    raise SystemExit(f"Expected one persisted WP-02 publication block, found {count}; refusing unsafe rewrite")

path.write_text(updated)
print("Safely transplanted authoritative V6 publication correction")
