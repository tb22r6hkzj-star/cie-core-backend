...
test("headwear is rejected when its crop behaves like surrounding hair",()=>{const dark=image([18,18,20],{x:5,y:1,w:10,h:4,color:[19,19,21]}); const r=run(hat,dark,"assist"); assert.equal(r.garment_zones.zones.accessory_jewelry,undefined); assert.equal(r.perception_v6.publication_decisions[0].reason,"hair_like_dark_region");});
