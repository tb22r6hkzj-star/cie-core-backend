...
test("contrasting headwear survives internal positive checks while market assist withholds it",()=>{const r=run(hat,image([18,18,20],{x:5,y:1,w:10,h:4,color:[20,80,160]}),"assist"); assert.equal(r.garment_zones.zones.accessory_jewelry,undefined); assert.equal(r.perception_v6.publication_decisions[0].published,true); assert.equal(r.perception_v6.object_presence.accessory_jewelry.present,true); assert.ok(r.perception_v6.publication_decisions[0].structural_evidence.length>0);});
