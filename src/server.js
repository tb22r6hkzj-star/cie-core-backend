...
function inferGarmentZones(
  ...
  const forceRegionOnly = !!options?.forceRegionOnly;
  ...
  for (const [zoneKey, fallbackColor] of Object.entries(zoneMap)) {
    ...
    if (forceRegionOnly && !hasSamRegion) {
      const emptyZoneRead = inferZoneColorRead(zoneKey, null, normalizedColors, [], true);
      zones[zoneKey] = {
        zone: zoneKey,
        ...emptyZoneRead,
      };
...
function buildOutfitAnalysis({ dominantHex, topColors, segmentedRegions = [], pipeline = null }) {
  ...
  const garmentZones = inferGarmentZones(
    ...
    segmentedRegions,
    {
      forceRegionOnly: !!pipeline?.sam_ok,
    }
  );
...
async function runSamSegmentation(imageUrl) {
  ...
  if (!replicateToken) {
    console.error("[SAM] Missing REPLICATE_API_TOKEN");
    return {
      enabled: false,
      ok: false,
  ...
  try {
    console.log("[SAM] starting segmentation");
    ...
    if (prediction?.status !== "succeeded") {
      console.log("[SAM] result:", false, 0);
      return {
        enabled: true,
        ok: false,
    ...
    console.log("[SAM] response received, raw segments:", parsedRegions.length);
    const enrichedRegions = await enrichSamRegionsWithMaskedColors(imageUrl, parsedRegions);
    console.log("[SAM] result:", true, enrichedRegions.length);
...
async function analyzeGhostColors(ghostUrl) {
  ...
  console.log("[SAM] result:", !!sam?.ok, (sam?.regions || []).length);
  ...
  return {
    ...
    pipeline: {
      ...
      segments_detected: Array.isArray(sam?.regions) ? sam.regions.length : 0,
      used_region_pipeline: !!sam?.ok,
    },
  };
}
  ...
  try {
    ...
    if (!analysis?.pipeline?.sam_ok) {
      return res.status(502).json({
        ...
        error: analysis?.pipeline?.sam_reason || "SAM failed during debug mode",
        pipeline: {
          sam_ok: false,
          segments_detected: Number(analysis?.pipeline?.segments_detected || 0),
          used_region_pipeline: false,
        },
      });
    }
    ...
    return res.json({
      ...
      pipeline: {
        ...
        segments_detected: Number(analysis?.pipeline?.segments_detected || 0),
        used_region_pipeline: !!analysis?.pipeline?.used_region_pipeline,
      },
