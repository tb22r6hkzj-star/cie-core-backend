...
function weightedMedoid(rows) {
  ...
  return rows
    .map((candidate) => ({
      candidate,
      cost: rows.reduce((sum, row) => sum + distance(candidate, row) * row.weight, 0),
    }))
 ...
 *
 * Raw measured hex values remain immutable evidence. This layer does not invent
 * a replacement color. It selects the measured sample that best represents the
 * stable material identity after strongly discounting luminance variation and
 * comparing linear-RGB chromaticity plus LAB chromatic direction.
 */
export function estimateGarmentIntrinsicColorV1(samples = []) {
  ...
  if (!rows.length) {
    return { available: false, version: "garment_color_constancy_v1", reason: "no_owned_measured_colors" };
  }

  const medoid = weightedMedoid(rows);
  const distances = rows.map((row) => ({ ...row, intrinsic_distance: distance(medoid, row) }));
  const threshold = Math.max(0.035, Math.min(0.09, weightedMean(distances, (row) => row.intrinsic_distance) * 1.8));
