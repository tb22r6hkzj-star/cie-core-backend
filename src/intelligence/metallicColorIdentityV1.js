...
export function classifyMeasuredMetallicPaletteV1({ colors = [], highlightRatio = 0, validationSupported = false } = {}) {
  ...
  if (!validationSupported || rows.length < 2) {
    return { publishable: false, family: null, display_name: null, confidence: 0, reason: "insufficient_validated_metallic_pixels" };
  }

  const warmShare = rows.reduce((sum, row) => {
    const { hue, saturation, lightness, r, g, b } = row.traits;
    const warmMetal = hue >= 24 && hue <= 62 && saturation >= 0.16 && lightness >= 0.20 && lightness <= 0.90 && r >= g && g > b;
    return sum + (warmMetal ? row.weight : 0);
  }, 0);
  ...
  return {
    ...
    evidence: { warm_pixel_share: warmShare, luminance_spread: luminanceSpread, highlight_ratio: clamp01(highlightRatio) },
  };
}
